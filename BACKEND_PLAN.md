...
- [x] **No detailed medical history collected** — the `message` field is capped at 500 characters, and both the UI copy (`messageHint`) and the schema comment explicitly say not to include medical history. There is no dedicated medical-history field anywhere in the schema.
